...
NZ_AGENT_SERVICE="/etc/systemd/system/fake-nezha-agent.service"
NZ_VERSION="v0.10.7"
...
modify_agent_config() {
    ...
    wget -t 2 -T 10 -O $NZ_AGENT_SERVICE https://${GITHUB_RAW_URL}/script/fake-nezha-agent.service >/dev/null 2>&1
    if [[ $? != 0 ]]; then
...
show_agent_log() {
    ...
    journalctl -xf -u fake-nezha-agent.service
...
uninstall_agent() {
    ...
    systemctl disable fake-nezha-agent.service
    systemctl stop fake-nezha-agent.service
...
restart_agent() {
    ...
    systemctl restart fake-nezha-agent.service
